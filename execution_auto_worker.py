#!/usr/bin/env python3
"""Isolated autonomous execution-learning worker for v58.6.4.
...
DIAG_PATH = Path(os.getenv('EXECUTION_AUTO_DIAGNOSTIC_PATH', 'data/execution_v58_6_4_latest_diagnostic.json'))
...
def main() -> int:
    ...
    try:
        ...
        _progress('backfill', phase='starting', limit=limit, processed=0, total=0)
        def _backfill_progress(stage, **extra):
            _progress(stage, **extra)
        backfill_result = backfill(limit=limit, dry_run=False, progress_callback=_backfill_progress, incremental=True)
        ...
        # The distributed model-training lease protects model mutation/publication,
        # not network-bound dataset refresh. Keeping backfill outside the lease avoids
        # blocking other instances for many minutes when historical providers are slow.
        from model_training_coordinator import training_slot
        _progress('lease-wait')
        with training_slot(owner='execution-auto-v58.6.4') as acquired:
            if not acquired:
                payload={'status':'training-slot-busy','started_at':started,'backfill':backfill_result,'finished_at':_now()}
        ...
        payload = {
            ...
            'backfill': {k:backfill_result.get(k) for k in ('status','shadow_written','shadow_skipped','paper_written','paper_skipped','unresolved','ambiguous','unsupported_symbols','errors','duration_seconds')},
