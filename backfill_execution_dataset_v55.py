"""V55 provider-aware first-hit replay and Paper/Shadow dataset unification.
...
def _request_json(s,url,params):
    r=s.get(url,params=params,timeout=float(os.getenv('EXECUTION_BACKFILL_HTTP_TIMEOUT','20'))); r.raise_for_status(); return r.json()

def _normalize(provider,data)->List[Tuple[int,float,float,float,int]]:
    out=[]
    try:
        if provider=='binance':
            rows=data or []
            ...
            for c in rows:
                o=int(c[0]); out.append((o,float(c[2]),float(c[3]),float(c[4]),o+60_000))
        elif provider=='okx':
            ...
            for c in rows:
                ...
                o=int(float(c[0])*1000); out.append((o,float(c[3]),float(c[4]),float(c[2]),o+60_000))
        elif provider=='mexc':
            d=(data or {}).get('data') or {}; times=d.get('time') or []
            for i,t in enumerate(times):
                o=int(float(t)*1000); out.append((o,float(d['high'][i]),float(d['low'][i]),float(d['close'][i]),o+60_000))
    except Exception:return []
    return sorted(out,key=lambda x:x[0])

def _fetch(provider,symbol,start,end,interval='5m',session=None):
    s=session or requests.Session(); a=int(start.timestamp()*1000); b=int(end.timestamp()*1000)
    try:
        if provider=='binance':
            data=_request_json(s,os.getenv('BINANCE_FUTURES_API_BASE','https://fapi.binance.com')+'/fapi/v1/klines',{'symbol':symbol,'interval':interval,'startTime':a,'endTime':b,'limit':1500})
        elif provider=='bybit':
            iv='1' if interval=='1m' else '5'; data=_request_json(s,os.getenv('BYBIT_API_BASE','https://api.bybit.com')+'/v5/market/kline',{'category':'linear','symbol':symbol,'interval':iv,'start':a,'end':b,'limit':1000})
        elif provider=='okx':
            bar='1m' if interval=='1m' else '5m'; inst=symbol.replace('USDT','-USDT-SWAP'); data=_request_json(s,'https://www.okx.com/api/v5/market/history-candles',{'instId':inst,'bar':bar,'after':a,'before':b,'limit':300})
        elif provider=='bitget':
            gran='1m' if interval=='1m' else '5m'; data=_request_json(s,'https://api.bitget.com/api/v2/mix/market/candles',{'symbol':symbol,'productType':'USDT-FUTURES','granularity':gran,'startTime':a,'endTime':b,'limit':1000})
        elif provider=='gate':
            iv='1m' if interval=='1m' else '5m'; contract=symbol.replace('USDT','_USDT'); data=_request_json(s,'https://api.gateio.ws/api/v4/futures/usdt/candlesticks',{'contract':contract,'interval':iv,'from':int(start.timestamp()),'to':int(end.timestamp())})
        elif provider=='mexc':
            iv='Min1' if interval=='1m' else 'Min5'; sym=symbol.replace('USDT','_USDT'); data=_request_json(s,f'https://contract.mexc.com/api/v1/contract/kline/{sym}',{'interval':iv,'start':int(start.timestamp()),'end':int(end.timestamp())})
        else:return []
        return _normalize(provider,data)
    except Exception:return []
...
def _candles_any(symbol,start,end,interval='5m',preferred=None,session=None):
    ...
    for provider in order:
        rows=_fetch(provider,symbol,start,end,interval,session); attempts.append({'provider':provider,'rows':len(rows),'interval':interval})
...
def backfill(limit=10000,dry_run=False):
    ...
    client=_client(); rich_by_fp=_learning_payload_map(limit)
    for row in shadows:
        try:
            created=_dt(row.get('created_at')); filled=_dt(row.get('filled_at')); status=str(row.get('status') or '').lower(); payload=row.get('payload') or {}; payload=json.loads(payload) if isinstance(payload,str) else (payload or {}); fp=str(payload.get('fingerprint') or ''); payload=_merge_payload(payload,rich_by_fp.get(fp))
            base={'sample_id':'shadow:'+str(row['id']),'source_id':row['id'],'sample_type':'SHADOW_EXECUTION' if filled else 'SHADOW_NO_FILL','decision_at_signal':str(payload.get('decisionAtSignal') or 'REJECTED'),'fingerprint':payload.get('fingerprint'),'symbol':row.get('symbol'),'direction':row.get('direction'),'setup':row.get('setup'),'source':row.get('source'),'signal_created_at':row.get('created_at'),'target_entry':row.get('target_entry'),'actual_entry':row.get('actual_entry'),'filled_at':row.get('filled_at'),'feature_payload':payload,'label_version':'first_hit_v55','sample_weight':1.0,'updated_at':datetime.now(timezone.utc).isoformat(),'ambiguous_same_candle':False}
            ...
            if not dry_run:client.table('execution_training_dataset_v55').upsert(base,on_conflict='sample_id').execute()
            done+=1
    ...
    for r in _paper_rows(limit):
        try:
            ...
            if not dry_run:client.table('execution_training_dataset_v55').upsert(sample,on_conflict='sample_id').execute()
            paper_written+=1
