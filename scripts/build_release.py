...
OUT=Path(sys.argv[1] if len(sys.argv)>1 else ROOT/'dist'/'Crypto-report-bot-v58.6.4-autonomous-watchdog.zip')
...
for p in ROOT.rglob('*'):
    ...
    if p.suffix not in ALLOW_SUFFIX and p.name not in {'VERSION','Dockerfile','Dockerfile.vps','.gitignore'}: continue
