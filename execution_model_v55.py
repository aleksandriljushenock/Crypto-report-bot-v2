"""V55 execution intelligence: unified Paper+Shadow dataset, purged OOS validation,
...
from execution_features_v55 import FEATURE_NAMES, extract, specialist_key

MODEL_PATH=Path(os.getenv('EXECUTION_MODEL_V55_PATH','data/execution_model_v55.joblib'))
...
def _load_rows(limit:int=20000)->List[Dict[str,Any]]:
    ...
    while len(out)<limit:
        rows=(_client().table('execution_training_dataset_v55').select('*').order('signal_created_at',desc=False).range(start,min(start+page-1,limit-1)).execute().data or [])
        if not rows:break
        out.extend(rows)
...
def _purged_split(rows, embargo_hours=None):
    """65/17/18 chronological split with a gap around both boundaries."""
    ...
    n=len(rows); i1=max(40,int(n*.65)); i2=max(i1+20,int(n*.82)); i2=min(i2,n-20)
    if i2<=i1:return None
    b1=_dt(rows[i1].get('signal_created_at')); b2=_dt(rows[i2].get('signal_created_at'))
    if not b1 or not b2:return rows[:i1],rows[i1:i2],rows[i2:]
    gap=timedelta(hours=emb)
    tr=[r for r in rows[:i1] if (_dt(r.get('exit_at')) or _dt(r.get('signal_created_at')) or b1) < b1-gap]
    ca=[r for r in rows[i1:i2] if (_dt(r.get('signal_created_at')) or b1) > b1+gap and (_dt(r.get('exit_at')) or _dt(r.get('signal_created_at')) or b2) < b2-gap]
    te=[r for r in rows[i2:] if (_dt(r.get('signal_created_at')) or b2) > b2+gap]
    return tr,ca,te
...
def _fit_one(rows,task,window,family='hgb',seed=55,feature_set='all'):
    ...
    tr,ca,te=split
    if min(len(tr),len(ca),len(te))<20:return None
    ytr=[label(r) for r in tr]; yca=[label(r) for r in ca]; yte=[label(r) for r in te]
    if len(set(ytr))<2 or len(set(yte))<2:return None
    indices=RAW_INDICES if feature_set=='raw' else ALL_INDICES
    X=[[extract(r)[i] for i in indices] for r in tr]; Xc=[[extract(r)[i] for i in indices] for r in ca]; Xt=[[extract(r)[i] for i in indices] for r in te]; sw=_weights(tr)
    ...
    raw=[float(x[1]) for x in clf.predict_proba(Xt)]; pred=[float(x) for x in cal.predict(raw)] if cal is not None else raw
    auc=_auc(yte,pred); brier=_brier(yte,pred)
    if task=='outcome':
        baseline=[max(.001,min(.999,extract(r)[2]/100.0)) for r in te]
        bauc=_auc(yte,baseline); bbrier=_brier(yte,baseline)
    else:
        prior=sum(ytr)/len(ytr); baseline=[prior]*len(yte); bauc=.5; bbrier=_brier(yte,baseline)
    auc_floor=float(os.getenv('EXECUTION_ML_MIN_AUC','0.55')); gain=float(os.getenv('EXECUTION_ML_MIN_AUC_GAIN','0.02')); bmax=float(os.getenv('EXECUTION_ML_MAX_BRIER','0.28'))
    ...
    if task=='outcome':
        target=[float(r.get('net_return_pct') or 0) for r in tr]; actual=[float(r.get('net_return_pct') or 0) for r in te]
        reg=_regressor(seed+100); reg.fit(X,target,sample_weight=sw); rp=[float(v) for v in reg.predict(Xt)]
        ...
        sign=sum((a>0)==(p>0) for a,p in zip(actual,rp))/len(actual)
        return_ok=bool(mae is not None and base_mae is not None and mae<base_mae*float(os.getenv('EXECUTION_RETURN_MAX_MAE_RATIO','0.97')) and (rho or -1)>=float(os.getenv('EXECUTION_RETURN_MIN_SPEARMAN','0.08')) and sign>=float(os.getenv('EXECUTION_RETURN_MIN_SIGN_ACCURACY','0.53')))
        reg_metrics={'return_mae':mae,'baseline_return_mae':base_mae,'return_spearman':rho,'return_sign_accuracy':sign,'return_runtime_ok':return_ok}
    return {'classifier':clf,'calibrator':cal,'regressor':reg,'window':window,'family':family,'feature_set':feature_set,'feature_indices':indices,'samples':len(usable),'train_samples':len(tr),'calibration_samples':len(ca),'validation':len(te),'auc':auc,'brier':brier,'baseline_auc':bauc,'baseline_brier':bbrier,'runtime_ok':runtime_ok,**reg_metrics}
...
def _cloud_path():return os.getenv('EXECUTION_MODEL_V55_CLOUD_PATH','v55/latest/execution_model_v55.joblib')
def _upload(path):
    try:
...
def train(trigger='manual'):
    ...
    bundle={'schema':55,'version':'execution-ensemble-v55-'+datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S'),'status':status,'trained_at':datetime.now(timezone.utc).isoformat(),'feature_names':FEATURE_NAMES,'models':models,'rows':len(rows),'trigger':trigger}
    ...
    return {'status':status,'version':bundle['version'],'rows':len(rows),'groups':list(models),'healthy_models':len(healthy),'best_auc':max((m.get('auc') or 0 for m in healthy),default=None),'cloud_saved':cloud}
...
def _ensemble(models,x):
    ...
    for m in models:
        ...
        try:
            indices=m.get('feature_indices') or ALL_INDICES; xx=[x[i] for i in indices]; p=float(m['classifier'].predict_proba([xx])[0][1]); cal=m.get('calibrator'); p=float(cal.predict([p])[0]) if cal is not None else p; ps.append(p); aucs.append(float(m.get('auc') or .5))
            if m.get('regressor') is not None and m.get('return_runtime_ok'):rs.append(float(m['regressor'].predict([xx])[0]))
        except Exception:continue
    if not ps:return None
    import statistics
    return {'probability':sum(ps)/len(ps)*100,'uncertainty':statistics.pstdev(ps)*100 if len(ps)>1 else 15.0,'expected_return_pct':sum(rs)/len(rs) if rs else None,'models':len(ps),'mean_auc':sum(aucs)/len(aucs) if aucs else None}
...
def predict(signal:Dict[str,Any])->Dict[str,Any]:
    ...
    x=extract(signal); outcome=_ensemble(g.get('outcome') or [],x); fill=_ensemble(g.get('fill') or [],x)
    if not outcome and g.get('key')!='GLOBAL':
        g=models.get('GLOBAL') or {}; outcome=_ensemble(g.get('outcome') or [],x); fill=_ensemble(g.get('fill') or [],x)
