...
def _paper_fill_verified(r):
    """Return True only when Paper has durable evidence that an entry really filled.

    Legacy rows may contain opened_at/entry_price even when the pending order was later
    cancelled without a market touch. V57 requires a real fill source and rejects known
    synthetic/invalid executions. This is the canonical truth rule for P(fill).
    """
    status=str(r.get('status') or '').lower()
    fill_source=str(r.get('fill_price_source') or '').strip().lower()
    verified=r.get('execution_verified')
    if verified is False or str(verified).lower() in {'false','0','no'}: return False
    if status in {'cancelled','expired','rejected','invalid'}: return False
    if not fill_source or fill_source in {'legacy_migrated_v38','phantom_midpoint','synthetic','unknown'}: return False
    return bool(r.get('opened_at')) and _num(r.get('entry_price'))>0


def _paper_sample(r):
    payload=r.get('signal_payload') or {}; payload=json.loads(payload) if isinstance(payload,str) else (payload or {})
    status=str(r.get('status') or '').lower(); filled=_paper_fill_verified(r)
    terminal_no_fill=status in {'cancelled','expired','rejected','invalid'} and not filled
    entry_status='filled' if filled else ('no_fill' if terminal_no_fill else 'unresolved')
