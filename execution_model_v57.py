...
def train(trigger='manual'):
    ...
    bundle={'schema':5864,'version':'execution-ensemble-v58.6.4-'+datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S'),'status':status,'trained_at':datetime.now(timezone.utc).isoformat(),'feature_names':FEATURE_NAMES,'models':models,'rows':len(rows),'trigger':trigger}
