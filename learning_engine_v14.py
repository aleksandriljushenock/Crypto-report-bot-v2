...
def _dataset_health(samples: Sequence[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    execution_count = sum(1 for s in samples if s.get('target_source') in {'paper_execution','shadow_execution_v55'})
...
def _execution_v55_cloud_samples() -> List[Dict[str, Any]]:
    """Load first-hit shadow execution labels produced by V55 replay."""
    if os.getenv("EXECUTION_V55_LEARNING_ENABLED", "true").lower() not in {"1","true","yes","on"}:
        return []
    try:
        from cloud_client import get_supabase_client
        client=get_supabase_client(); out=[]; offset=0; cap=int(_runtime_env("EXECUTION_V55_LEARNING_MAX_ROWS","10000")); page=1000
        while len(out)<cap:
            rows=(client.table("execution_training_dataset_v55").select("*").eq("entry_status","filled").order("signal_created_at",desc=False).range(offset,min(offset+page-1,cap-1)).execute().data or [])
            if not rows: break
            for row in rows:
                ...
                out.append({"fingerprint":str(row.get("fingerprint") or row.get("shadow_id")),"symbol":row.get("symbol"),"timeframe":payload.get("timeframe") or payload.get("interval") or "multi","direction":_normalize_direction(row.get("direction")),"setup":str(row.get("setup") or payload.get("setup") or "NONE").upper(),"created_at":row.get("signal_created_at"),"old_score":float(payload.get("aiScore") or payload.get("score") or 0),"factors":{k:float(factors.get(k,50)) for k in FEATURES},"returns":{},"return":ret,"win":1.0 if ret>1e-9 else 0.0,"r_multiple":float(row.get("r_multiple") or 0),"target_horizon":"execution","target_source":"shadow_execution_v55","close_reason":outcome})
...
def load_samples() -> List[Dict[str, Any]]:
    ...
    # V55 replayed shadow executions are independent execution-labelled samples.
    # They are merged by fingerprint and override proxy labels.
    v55_exec={str(x.get("fingerprint")):x for x in _execution_v55_cloud_samples() if x.get("fingerprint")}
    ...
    for item in grouped_samples:
        ...
        if item.get("target_source") == "shadow_execution_v55" and item.get("return") is not None:
            pass
        elif exec_target:
...
def _sample_weight(sample: Dict[str, Any]) -> float:
    ...
    execution_boost = max(1.0, float(_runtime_env("LEARNING_EXECUTION_SAMPLE_WEIGHT", "4.0"))) if sample.get("target_source") in {"paper_execution","shadow_execution_v55"} else 1.0
