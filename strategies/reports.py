...
def winrate_text(strategy: str = "fib_05_pullback"):
    ...
    lines = [
        ...
        f"Побед / убытков / BE: <b>{s['wins']} / {s['losses']} / {s.get('breakeven',0)}</b>",
        f"Win Rate: <b>{s['win_rate']:.1f}%</b>",
        f"Profit Factor: <b>{pf_text}</b>",
        f"Expectancy / средний return: <b>{s['expectancy']:+.2f}%</b>",
        f"Return: <b>{s.get('cumulative_return',0):+.2f}%</b> · Compounded: <b>{s.get('compounded_return',0):+.2f}%</b>",
        f"Max DD: <b>{s.get('max_drawdown',0):.2f}%</b>",
        "",
