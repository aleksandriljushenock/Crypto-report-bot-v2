...
SPECS: tuple[SettingSpec, ...] = (
    ...
    SettingSpec("TRADE_TOP_LIQUID_SYMBOLS", "int", 80, "runtime", "Монет в скане", "Количество наиболее ликвидных монет для анализа.", 1, 500),
    SettingSpec("TRADE_SCAN_BATCH_SIZE", "int", 8, "runtime", "Размер batch скана", "Сколько монет держать в памяти одновременно. Меньше = ниже RAM.", 2, 20),
    SettingSpec("FAST_SCAN_POOL_SIZE", "int", 250, "runtime", "Fast Scan pool", "Сколько ликвидных рынков быстро ранжировать до Deep Scan.", 50, 2000),
    SettingSpec("TRADE_SCAN_MAX_WORKERS", "int", 2, "runtime", "Параллельность", "Число одновременных задач Deep Scan. Больше ускоряет, но повышает RAM и API load.", 1, 8),
    ...
    SettingSpec("NEAR_SIGNAL_RESCAN_LIMIT", "int", 24, "runtime", "Near лимит", "Максимум Near-кандидатов на один быстрый повторный проход.", 1, 100),
    ...
    SettingSpec("HEDGE_CANDIDATE_POOL", "int", 28, "runtime", "Hedge-кандидатов", "Количество кандидатов для тяжёлого Hedge-этапа.", 1, 100),
    ...
    SettingSpec("STRATEGY_LAB_PARALLEL_MAX_SYMBOLS", "int", 80, "runtime", "Strategy Lab parallel лимит", "Максимум монет одной стратегии, когда одновременно работает основной Deep Scan.", 10, 300),
    ...
    SettingSpec("CHRONOS_ENABLED", "bool", False, "optimizer", "Chronos", "Включить Chronos для финальных кандидатов. Состояние хранится в Supabase."),
